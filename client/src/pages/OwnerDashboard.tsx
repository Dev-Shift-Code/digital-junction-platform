...
const weeklyPreviewValues = [12400, 18600, 14900, 24100, 17800, 28600, 22300];
...
const monthlyPreviewValues = [88500, 76200, 101400, 93400, 122600, 111800, 143200, 128500, 159400, 146100, 176800, 168900];
          ...
          <>
            ...
            <section className="grid gap-5 xl:grid-cols-[1.45fr_.85fr]">
              <ChartCard title="Sales Overview" subtitle="Live revenue visualisation will appear when sales exist." className="min-h-[20rem]">
                <div className="grid h-52 grid-cols-7 items-end gap-2 rounded-2xl border border-dashed border-[#428475]/22 bg-[#FFF4E1]/45 p-5" aria-label="Sales overview sample chart placeholder">
                  {weeklyBars.map((height, index) => <PreviewBar key={weeklyLabels[index]} compact label={weeklyLabels[index]} value={weeklyPreviewValues[index]} height={height} shortLabel={weeklyLabels[index]?.slice(0, 1) ?? ""} />)}
                </div>
                <p className="mt-4 text-xs leading-5 text-[#1A312C]/58"><span className="font-bold text-[#428475]">Sample chart shape only. Hover or focus any bar to see its sample value.</span> These values are a design preview, not a sales total, revenue figure, or recorded result.</p>
              </ChartCard>
            ...
            <section className="grid gap-5 lg:grid-cols-2">
              <ChartCard title="Weekly Sales" subtitle="A week-at-a-glance view for future real sales.">
                <div className="mt-6 grid grid-cols-7 items-end gap-3" aria-label="Weekly sales sample chart placeholder">{weeklyBars.map((height, index) => <PreviewBar key={weeklyLabels[index]} label={weeklyLabels[index]} value={weeklyPreviewValues[index]} height={height} shortLabel={weeklyLabels[index]?.slice(0, 3) ?? ""} />)}</div>
                <p className="mt-4 text-xs text-[#1A312C]/58">Hover or focus a bar for its sample amount · real weekly values are not recorded yet.</p>
              </ChartCard>
              <ChartCard title="Monthly Revenue" subtitle="Monthly revenue remains unavailable until paid records exist.">
                <div className="relative mt-6 h-36 overflow-hidden rounded-xl border border-dashed border-[#428475]/22 bg-[#FFF4E1]/45 p-4" aria-label="Monthly revenue sample chart placeholder"><svg className="absolute inset-4 h-[calc(100%-2rem)] w-[calc(100%-2rem)]" viewBox="0 0 320 96" preserveAspectRatio="none" aria-hidden="true"><polyline points={monthlyTrend.map((point, index) => `${(index / (monthlyTrend.length - 1)) * 320},${96 - point}`).join(" ")} fill="none" stroke="#428475" strokeWidth="3" strokeDasharray="7 6" opacity="0.55" /><line x1="0" y1="78" x2="320" y2="78" stroke="#1A312C" strokeOpacity="0.14" /></svg>{monthlyTrend.map((point, index) => <PreviewPoint key={monthlyLabels[index]} label={monthlyLabels[index]} value={monthlyPreviewValues[index]} x={(index / (monthlyTrend.length - 1)) * 100} y={(point / 96) * 100} edge={index < 2 ? "start" : index > 9 ? "end" : "center"} />)}<div className="pointer-events-none absolute inset-x-4 bottom-3 flex justify-between font-mono text-[0.52rem] uppercase text-[#1A312C]/42"><span>Jan</span><span>Apr</span><span>Aug</span><span>Dec</span></div></div>
                <p className="mt-4 text-xs text-[#1A312C]/58">Hover or focus a point for its sample monthly value · no real revenue amount is represented.</p>
              </ChartCard>
            </section>
...
function PreviewBar({ label, value, height, shortLabel, compact = false }: { label: string; value: number; height: number; shortLabel: string; compact?: boolean }) { return <div className={`group relative flex h-full flex-col justify-end gap-2 ${compact ? "" : "min-w-0"}`}><span className="pointer-events-none absolute bottom-[calc(100%+0.55rem)] left-1/2 z-20 w-40 -translate-x-1/2 rounded-lg bg-[#1A312C] px-3 py-2 text-center text-[0.62rem] leading-4 text-[#FFF4E1] opacity-0 shadow-xl transition group-hover:opacity-100 group-focus-within:opacity-100"><strong className="block text-[#89D7B7]">{formatPreviewPeso(value)}</strong>{label} · Sample value<br /><span className="text-[#FFF4E1]/62">Not a recorded sale</span></span><button type="button" className={`group/bar relative block w-full rounded-t-xl bg-[#1A312C]/7 text-left outline-none focus-visible:ring-2 focus-visible:ring-[#428475] focus-visible:ring-offset-2 ${compact ? "h-full" : "h-28"}`} aria-label={`${label}: sample value ${formatPreviewPeso(value)}. Not a recorded sale.`} title={`${label}: ${formatPreviewPeso(value)} sample value — not a recorded sale.`}><span className={`absolute inset-x-0 bottom-0 rounded-t-xl bg-[#428475]/42 transition group-hover/bar:bg-[#428475]/70 ${compact ? "" : ""}`} style={{ height: `${height}%` }} /></button><span className={`text-center font-mono text-[#1A312C]/46 ${compact ? "text-[0.52rem] uppercase" : "text-[0.56rem]"}`}>{shortLabel}</span></div>; }
function PreviewPoint({ label, value, x, y, edge }: { label: string; value: number; x: number; y: number; edge: "start" | "center" | "end" }) { const position = edge === "start" ? "left-0" : edge === "end" ? "right-0" : "left-1/2 -translate-x-1/2"; return <div className="group absolute z-10" style={{ left: `${x}%`, bottom: `calc(${y}% - 0.35rem)` }}><button type="button" className="grid size-4 -translate-x-1/2 translate-y-1/2 place-items-center rounded-full border-2 border-[#FFF4E1] bg-[#428475] outline-none transition hover:scale-125 focus-visible:scale-125 focus-visible:ring-2 focus-visible:ring-[#1A312C] focus-visible:ring-offset-2" aria-label={`${label}: sample value ${formatPreviewPeso(value)}. Not a recorded revenue value.`} title={`${label}: ${formatPreviewPeso(value)} sample value — not recorded revenue.`}><span className="sr-only">{label} sample value</span></button><span className={`pointer-events-none absolute bottom-[calc(100%+0.7rem)] z-20 w-44 rounded-lg bg-[#1A312C] px-3 py-2 text-center text-[0.62rem] leading-4 text-[#FFF4E1] opacity-0 shadow-xl transition group-hover:opacity-100 group-focus-within:opacity-100 ${position}`}><strong className="block text-[#89D7B7]">{formatPreviewPeso(value)}</strong>{label} · Sample value<br /><span className="text-[#FFF4E1]/62">Not recorded revenue</span></span></div>; }
function formatPreviewPeso(value: number) { return `₱${new Intl.NumberFormat("en-PH").format(value)}`; }
