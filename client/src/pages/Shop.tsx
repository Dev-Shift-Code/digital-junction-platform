...
import { parseEditableKeyValues } from "@/data/publicContentStructured";
...
export default function Shop() {
  ...
  const filterContent = usePublicSection("shop", "filter-controls", getPublicSectionDefault("shop", "filter-controls"));
  const filterCopy = parseEditableKeyValues(filterContent.body ?? "", { allLabel: "All products", searchPlaceholder: "Search products", sortLabel: "Sort by", sortNewest: "Newest first", sortPriceLow: "Price: low to high", sortPriceHigh: "Price: high to low", sortTitle: "Name: A–Z", filterLabel: "Filter", categoryLabel: "Product category", clearLabel: "Clear", filterHint: "Use the filters to find the right Digital Junction product.", emptyTitle: "No products are published yet.", emptyBody: "Published Digital Junction products will appear here." });
            ...
            <div className="site-container relative grid gap-10 py-16 sm:py-20 lg:grid-cols-[1.05fr_.95fr] lg:items-end lg:py-24">
              <div className="max-w-2xl">
                ...
                <div className="mt-9 flex flex-wrap gap-2">
                  <button type="button" onClick={() => setSelectedCategories([])} className={`rounded-full px-4 py-2 text-xs font-bold transition ${selectedCategories.length === 0 ? "bg-[#89D7B7] text-[#1A312C]" : "border border-[#FFF4E1]/20 bg-[#FFF4E1]/7 text-[#FFF4E1] hover:bg-[#FFF4E1]/14"}`}>{filterCopy.allLabel}</button>
          ...
          <section className="section-grid py-16 sm:py-20">
            <div className="site-container">
              ...
              <div className="mt-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <label className="relative block w-full max-w-md"><Search className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#428475]" /><input value={search} onChange={event => setSearch(event.target.value)} className="form-field !border-[#1A312C]/18 !bg-[#FFF4E1] !pl-11 text-sm shadow-[0_8px_20px_rgba(26,49,44,.04)]" placeholder={filterCopy.searchPlaceholder} /></label>
                <label className="flex items-center gap-3 self-end text-xs font-semibold text-[#1A312C]/65 lg:self-auto">{filterCopy.sortLabel} <select value={sort} onChange={event => setSort(event.target.value as SortOption)} className="rounded-lg border border-[#1A312C]/15 bg-[#89D7B7]/22 px-3 py-2 font-semibold text-[#1A312C] outline-none focus-visible:ring-2 focus-visible:ring-[#428475]"><option value="newest">{filterCopy.sortNewest}</option><option value="price-low">{filterCopy.sortPriceLow}</option><option value="price-high">{filterCopy.sortPriceHigh}</option><option value="title">{filterCopy.sortTitle}</option></select></label>
              </div>

              <div className="mt-7 grid gap-6 lg:grid-cols-[13rem_1fr]">
                <aside className="h-fit rounded-[1.2rem] bg-[#1A312C] p-5 text-[#FFF4E1] lg:sticky lg:top-24"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-sm font-bold"><Filter className="size-4 text-[#89D7B7]" />{filterCopy.filterLabel}</span>{(search || selectedCategories.length || sort !== "newest") ? <button type="button" onClick={clearFilters} className="inline-flex items-center gap-1 text-[.65rem] font-bold text-[#89D7B7] hover:text-[#FFF4E1]"><X className="size-3" />{filterCopy.clearLabel}</button> : null}</div><p className="mt-7 font-mono text-[.55rem] uppercase tracking-[.11em] text-[#FFF4E1]/46">{filterCopy.categoryLabel}</p><div className="mt-4 grid gap-3">{categories.length ? categories.map(category => <label key={category} className="flex cursor-pointer items-center gap-2.5 text-xs leading-5 text-[#FFF4E1]/77"><input checked={selectedCategories.includes(category)} onChange={() => toggleCategory(category)} type="checkbox" className="size-3.5 accent-[#89D7B7]" />{category}</label>) : <p className="text-xs leading-5 text-[#FFF4E1]/58">Categories will appear when a product is published.</p>}</div><div className="mt-8 rounded-xl bg-[#428475] p-3.5"><SlidersHorizontal className="size-4 text-[#89D7B7]" /><p className="mt-3 text-[.68rem] leading-5 text-[#FFF4E1]/80">{filterCopy.filterHint}</p></div></aside>
                <section>{isLoading ? <div className="grid min-h-80 place-items-center rounded-[1.25rem] border border-[#428475]/16 bg-[#89D7B7]/15"><Loader2 className="size-7 animate-spin text-[#428475]" /></div> : error ? <div className="rounded-[1.25rem] border border-dashed border-[#1A312C]/22 bg-[#89D7B7]/14 p-10 text-center"><h2 className="display text-3xl text-[#1A312C]">The catalogue needs a moment.</h2><p className="mt-3 text-sm text-[#1A312C]/60">Please refresh and try again.</p></div> : filteredProducts.length ? <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{filteredProducts.map(product => <article key={product.id} className="group overflow-hidden rounded-[1.25rem] border border-[#1A312C]/14 bg-[#FFF4E1] shadow-[0_12px_28px_rgba(26,49,44,.055)] transition duration-200 hover:-translate-y-1 hover:border-[#428475]/55 hover:shadow-[0_20px_36px_rgba(26,49,44,.12)]">{product.coverImageUrl ? <Link href={`/shop/${product.slug}`} className="block aspect-[1.25] overflow-hidden bg-[#89D7B7]/18"><img src={product.coverImageUrl} alt={product.title} className="size-full object-cover transition duration-300 group-hover:scale-[1.035]" /></Link> : <Link href={`/shop/${product.slug}`} className="relative block aspect-[1.25] overflow-hidden bg-[linear-gradient(135deg,#1A312C,#428475)]"><span className="absolute left-4 top-4 rounded-full bg-[#FFF4E1]/12 px-2.5 py-1 font-mono text-[.52rem] uppercase tracking-[.1em] text-[#FFF4E1]">{product.category}</span></Link>}<div className="p-5"><p className="font-mono text-[.55rem] uppercase tracking-[.1em] text-[#428475]">{product.category}</p><h3 className="mt-3 text-base font-bold leading-6 text-[#1A312C]">{product.title}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-[#1A312C]/60">{product.summary}</p><div className="mt-6 flex items-end justify-between gap-3 border-t border-[#1A312C]/10 pt-4"><div><p className="font-mono text-[.5rem] uppercase tracking-[.08em] text-[#1A312C]/45">Price</p><p className="mt-1 font-mono text-base font-bold text-[#1A312C]">{money(product.price)}</p></div><Link href={`/shop/${product.slug}`} className="inline-flex items-center gap-1.5 rounded-lg bg-[#428475] px-3 py-2 text-xs font-bold text-[#FFF4E1] transition hover:bg-[#1A312C]">View details<ArrowRight className="size-3.5" /></Link></div></div></article>)}</div> : <div className="rounded-[1.25rem] border border-dashed border-[#428475]/35 bg-[#89D7B7]/16 px-7 py-16 text-center"><p className="eyebrow">{products.length ? "No matches" : "Digital products"}</p><h2 className="display mt-3 text-3xl text-[#1A312C]">{products.length ? "No products match those filters." : filterCopy.emptyTitle}</h2><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#1A312C]/62">{products.length ? "Try another category or clear your search." : filterCopy.emptyBody}</p>{products.length ? <button type="button" onClick={clearFilters} className="button-primary mt-6">{filterCopy.clearLabel}</button> : null}</div>}</section>
              </div>
            </div>
          </section>
        )}
