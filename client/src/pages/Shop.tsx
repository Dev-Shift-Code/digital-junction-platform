          ...
          <section className="section-grid py-16 sm:py-20">
            <div className="site-container">
              ...
              <div className="mt-7 grid gap-6 lg:grid-cols-[13rem_1fr]">
                ...
                <section>{isLoading ? <div className="grid min-h-80 place-items-center rounded-[1.25rem] border border-[#428475]/16 bg-[#89D7B7]/15"><Loader2 className="size-7 animate-spin text-[#428475]" /></div> : error ? <div className="rounded-[1.25rem] border border-dashed border-[#1A312C]/22 bg-[#89D7B7]/14 p-10 text-center"><h2 className="display text-3xl text-[#1A312C]">The catalogue needs a moment.</h2><p className="mt-3 text-sm text-[#1A312C]/60">Please refresh and try again.</p></div> : filteredProducts.length ? <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{filteredProducts.map(product => <article key={product.id} className="group overflow-hidden rounded-[1.25rem] border border-[#1A312C]/14 bg-[#FFF4E1] shadow-[0_12px_28px_rgba(26,49,44,.055)] transition duration-200 hover:-translate-y-1 hover:border-[#428475]/55 hover:shadow-[0_20px_36px_rgba(26,49,44,.12)]">{product.coverImageUrl ? <Link href={`/shop/${product.slug}`} className="block aspect-[1.25] overflow-hidden bg-[#89D7B7]/18"><img src={product.coverImageUrl} alt={product.title} className="size-full object-cover transition duration-300 group-hover:scale-[1.035]" /></Link> : <Link href={`/shop/${product.slug}`} className="relative block aspect-[1.25] overflow-hidden bg-[linear-gradient(135deg,#1A312C,#428475)]"><span className="absolute left-4 top-4 rounded-full bg-[#FFF4E1]/12 px-2.5 py-1 font-mono text-[.52rem] uppercase tracking-[.1em] text-[#FFF4E1]">{product.category}</span></Link>}<div className="p-5"><p className="font-mono text-[.55rem] uppercase tracking-[.1em] text-[#428475]">{product.category}</p><h3 className="mt-3 text-base font-bold leading-6 text-[#1A312C]">{product.title}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-[#1A312C]/60">{product.summary}</p><div className="mt-6 grid gap-3 border-t border-[#1A312C]/10 pt-4"><div className="flex items-end justify-between gap-3"><div><p className="font-mono text-[.5rem] uppercase tracking-[.08em] text-[#1A312C]/45">Price</p><p className="mt-1 font-mono text-base font-bold text-[#1A312C]">{money(product.price)}</p></div><Link href={`/shop/${product.slug}`} className="inline-flex items-center gap-1.5 rounded-lg bg-[#428475] px-3 py-2 text-xs font-bold text-[#FFF4E1] transition hover:bg-[#1A312C]">Purchase Here<ArrowRight className="size-3.5" /></Link></div><ProductPurchaseActions product={product} showGcash={false} /></div></div></article>)}</div> : <div className="rounded-[1.25rem] border border-dashed border-[#428475]/35 bg-[#89D7B7]/16 px-7 py-16 text-center"><p className="eyebrow">{products.length ? "No matches" : "Digital products"}</p><h2 className="display mt-3 text-3xl text-[#1A312C]">{products.length ? "No products match those filters." : "No products are published yet."}</h2><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#1A312C]/62">{products.length ? "Try another category or clear your search." : "Published Digital Junction products will appear here."}</p>{products.length ? <button type="button" onClick={clearFilters} className="button-primary mt-6">Clear filters</button> : null}</div>}</section>
              </div>
            </div>
          </section>
        )}
