...
export default function OwnerSalesManagement() {
  ...
  const revokeDelivery = (orderId: number) => {
    if (!window.confirm("Revoke all unused one-time buyer download links for this order? A buyer will no longer be able to use any existing unused delivery link.")) return;
    setError("");
    revokeDeliveryLinks.mutate({ orderId }, { onError: issue => setError(issue.message || "Delivery links could not be revoked.") });
  };
