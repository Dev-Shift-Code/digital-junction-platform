import { AXIOS_TIMEOUT_MS, COOKIE_NAME, ONE_YEAR_MS, OWNER_SESSION_COOKIE, decodeOAuthState } from "@shared/const";
...
class SDKServer {
  ...
  async authenticateRequest(req: Request, cookieName = COOKIE_NAME): Promise<AuthenticatedUser> {
    ...
    if (!sessionToken && cookieName === COOKIE_NAME) {
    ...
    if (!sessionToken && cookieName === OWNER_SESSION_COOKIE) {
      throw ForbiddenError("Missing owner session cookie");
    }
